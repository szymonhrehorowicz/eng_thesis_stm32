...
static double b[FILTER_ORDER + 1] = {2.94123952e-09, 2.05886766e-08, 6.17660299e-08, 1.02943383e-07,
                                     1.02943383e-07, 6.17660299e-08, 2.05886766e-08, 2.94123952e-09};
static double a[FILTER_ORDER + 1] = {1.0,         -6.43532476,  17.76978865, -27.29100833,
                                     25.17605177, -13.94983801, 4.2985741,   -0.56824305};
...
void IIR_update(IIRfilter_t *self, float current_value)
{
    ...
    self->x[0] = current_value;

    self->y[0] = 0.0;
    for (int i = 0; i <= FILTER_ORDER; i++)
    {
        self->y[0] += b[i] * self->x[i];
        if (i > 0)
        {
            self->y[0] -= a[i] * self->y[i];
        }
    }
...
{
    //    const double ff  = freq / (1.0f / (self->sample_time / 1000.0f));
    //    const double ita = 1.0f / tan(M_PI * ff);
    //    const double q   = sqrt(2.0f);
    //    self->b0 = 1.0f / (1.0f + (q * ita) + (ita * ita));
    //    self->b1 = 2.0f * self->b0;
    //    self->b2 = self->b0;
    //    self->a1 = 2.0f * ((ita * ita ) - 1.0f) * self->b0;
    //    self->a2 = -(1.0f - (q * ita) + (ita * ita)) * self->b0;
}
